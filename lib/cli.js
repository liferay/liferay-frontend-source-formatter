...
CLI.prototype = _.create(
	EventEmitter.prototype,
	{
		...
		init: function() {
			...
			cfgPromise.then(
				function(config) {
					...
					if (obj) {
						if (config.flags) {
							_.merge(instance.flags, config.flags);
						}

						if (!instance.flags.filenames) {
							instance._log('Using local config from ', obj.filepath);
						}
					}
					else if (err && instance.flags.verbose) {
						instance._log('Could not resolve any local config: ', err, err.stack);
					}
		...
		afterFormat: function(err, results) {
			var instance = this;
			...
			if (instance.flags.inlineEdit) {
				series = results.reduce(
					function(prev, item, index) {
						if (item && item.contents !== item.data) {
							prev.push(instance.writeFile.bind(instance, item.file, item.contents));
						}
		...
		formatFile: function(file, data) {
			...
			if (formatter) {
				...
				contents = this.processFileData(data, formatter);
		...
		onRead: function(err, data, file, done) {
			...
			if (err) {
				...
				results = this.formatFile(file, data);
			}
		...
		_start: function() {
			...
			if (instance.flags.junit) {
				series.push(instance.createReport.bind(instance));
			}


			instance._async.series(series, instance.afterFormat.bind(instance));

...
var filterFileErrors = function(errors) {
	return _.reject(errors, ['type', 'ignored']);
};
...
class CLI extends EventEmitter {
	...
	_loadConfig(config) {
		if (this.flags.config) {
			...
			if (this.flags.verbose && !this.flags.filenames) {
				if (obj) {
	...
	_loadConfigs() {
		...
		return Promise.reduce(
			...
			function(prev, item, index) {
				...
				return res.then(
					function(config) {
						prev[item] = instance._loadConfig(config);

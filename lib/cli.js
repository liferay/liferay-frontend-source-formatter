...
var API = {
	...
	init: function() {
		...
		if (flags.relative) {
			series.unshift(instance.setRelativePath.bind(this));
		}
	...
	processFileData: function(data, formatter, done) {
		...
		if (contents !== data && flags.inlineEdit) {
	...
	writeFile: function(file, contents, done) {
		this._write(file, contents, A.rbind(this, this.writeFile, done));
	},

	_args: argv._,
	_cwd: base.CWD,
	_async: async,
	_exec: cli.exec.bind(cli),
	_generateJUnit: junit.generate,
	_log: console.log.bind(console),
	_logger: Logger,
	_read: fs.readFile.bind(fs),
	_write: fs.writeFile.bind(fs)
};

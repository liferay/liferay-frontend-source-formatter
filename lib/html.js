			...
			format: function(contents) {
				...
				return iterateLines(
					...
					function(item, index, collection) {
						...
						re.hasExtraNewLines(item, index, collection, filePath);
			...
			_attrCheckOrder: function(attrName, lastAttr, line, lineNum) {
				if (lastAttr > attrName) {
					...
					if (!note || note && VERBOSE) {
						trackErr(sub('Line {0} Sort attributes{3}: {1} {2}', lineNum, lastAttr, attrName, note).warn, filePath);
					}
				}
			},
					...
					function(item, index, collection) {
						...
						if (/^[A-Za-z]/.test(item)) {
							...
							if (!onAttr && !labelAttr && lastAttrPiece > item) {
								trackErr(sub('Line {0} Sort attribute values: {1} {2}', lineNum, lastAttrPiece, item).warn, filePath);

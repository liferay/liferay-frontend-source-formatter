...
Formatter.HTML = Formatter.create(
	{
		extensions: /\.(jsp.*|htm.*|vm|ftl|tag|tpl|tmpl)$/,
		id: 'html',
		prototype: {
			...
			extractCSS: function(contents) {
				...
				var hasCSS = (re.REGEX_STYLE).test(contents);

				if (hasCSS) {
					var reStyleGlobal = new RegExp(re.REGEX_STYLE.source, 'g');
			...
			format: function(contents) {
				...
				return iterateLines(
					...
					function(item, index, collection) {
						...
						fullItem = re.iterateRules('html', fullItem, context);
			...
			formatCSS: function(styleBlocks) {
				var cssFormatter = new Formatter.CSS(this.file, this.logger);
			...
			formatJs: function(scriptBlocks) {
				var jsFormatter = new Formatter.JS(this.file, this.logger);
			...
			_attrRemoveScriptlets: function(line, lineNum) {
				...
				var mapTokens = function(item, index, collection) {
				...
				var jstlM = line.match(/\$\{.*?\}/g);

				var jstlMatches = jstlM && jstlM.map(mapTokens);

				if (matches && jstlMatches) {
					matches = matches.concat(jstlMatches);
				}

				var attrMapEntry = this._attrGetMapEntry(lineNum);

				attrMapEntry.matches = matches || jstlMatches;
			...
			_jsIterateRules: function(scriptBlock) {
				...
				iterateLines(
					...
					function(item, index) {
						...
						fullItem = re.iterateRules('htmlJS', fullItem, context);
					}
				);

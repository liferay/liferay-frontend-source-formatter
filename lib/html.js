...
Formatter.HTML = Formatter.create(
	{
		...
		id: 'html',
		prototype: {
			extractJs: function(contents, file) {
				var scriptBlocks = [];
			...
			format: function(contents, file) {
				...
				instance.parseJs(contents, file);
				...
				return iterateLines(
					...
					function(item, index, collection) {
						...
						filteredItem = instance._processAttrs(filteredItem, lineNum, file);
			...
			formatJS: function(scriptBlocks, file) {
				var jsFormatter = new Formatter.JS(file);
				var esLintConfig = require('./eslint_config_jsp');

				return scriptBlocks.map(
					function(item, index) {
						jsFormatter.format(item.contents, file, esLintConfig);
					}
				);
			},

			parseJs: function(contents, file) {
				var scriptBlocks = this.extractJs(contents, file);

				scriptBlocks = this.sanitizeScriptBlocks(scriptBlocks);
				scriptBlocks = this.formatJS(scriptBlocks, file);
			},
			...
			_attrCheckOrder: function(attrName, lastAttr, line, lineNum, file) {
			...
			_attrSortValues: function(attrName, attrValue, item, line, lineNum, file) {
			...
			_processAttrs: function(line, lineNum, file) {
				var instance = this;
						...
						if (attrs) {
							...
							attrs.forEach(
								function(item, index, collection) {
									...
									instance._attrCheckOrder(attrName, lastAttr, line, lineNum, file);

									item = instance._attrSortValues(attrName, attrValue, item, line, lineNum, file);

...
re.prototype = {
	...
	getMessage: function(lineNum, result, rule, context) {
		var warning;
		...
		if (_.isString(message)) {
			warning = this.message(message, lineNum, result, rule, context);
		}
		else if (_.isFunction(message)) {
			warning = message.call(this, lineNum, result, rule, context);
		}
			...
			_.forEach(
				...
				function(rule, ruleName) {
					if (instance.isValidRule(ruleName, rule, context)) {
						...
						if (result) {
							var message = instance.getMessage(lineNum, result, rule, context);
	...
	message: function(message, lineNum, result, rule, context) {
		return sub(message, lineNum, context.item);
	},

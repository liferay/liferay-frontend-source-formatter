...
module.exports = {
	...
	hexRedundant: {
		message: function(lineNum, result, rule, context) {
			var match = this.hasHex(context.item);

			var message = 'Hex code can be reduced to ' + rule._reduceHex(match) + ': {1}';

			return this.message(message, lineNum, result, rule, context);
		},
	...
	missingNewlines: {
		message: function(lineNum, result, rule, context) {
			...
			message = this.message(message, lineNum, result, rule, context);
	...
	trailingNewlines: {
		...
		message: function(lineNum, result, rule, context) {
			...
			return this.message('Needless new line', lineNum + offset, result, rule, context);
		},
		test: function(item, regex, rule, context) {
			var hasCloser = this.REGEX_BRACE_CLOSING.test(item);
			var hasOpener = this.REGEX_BRACE_OPENING.test(item);

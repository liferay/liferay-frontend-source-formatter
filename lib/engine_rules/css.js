...
module.exports = {
	...
	missingListValuesSpace: {
		...
		test: function(item, regex) {
			...
			if (this.hasProperty(item)) {
				var value = item.match(REGEX.PROP_KEY);
	...
	missingNewlines: {
		...
		test: function(item, regex, rule, context) {
			...
			var hasCloser = REGEX.BRACE_CLOSING.test(item);
			var hasOpener = REGEX.BRACE_OPENING.test(item);
		...
		_isNextLineInvalid: function(item) {
			return (item !== '' && !REGEX.BRACE_CLOSING.test(item) && item.indexOf('@') !== 0);
		},

		_isPrevLineInvalid: function(item) {
			return (item !== '' && !REGEX.BRACE_OPENING.test(item) && !REGEX.CSS_COMMA_END.test(item));
		}
	},

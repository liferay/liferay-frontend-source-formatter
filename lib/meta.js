...
var readFile = function(filePath, fileName, moduleInfo, cb) {
	fs.readFile(
		filePath,
		...
		function(err, files) {
			...
			files.forEach(
				function(item, index) {
					fileSeries.push(
						function(cb) {
							readFile(path.join(liferayModuleDir, item), item, moduleInfo, cb);
						}
					);
				}
			);
			...
			async.series(
				...
				function(results) {
					...
					var combined = A.Array.dedupe(moduleKeys.concat(fileModuleKeys));
								...
								if (!moduleMeta.requires && fileMeta.requires) {
									...
									if (largeReq.join() !== smallReq.join()) {
										...
										if (VERBOSE) {
											...
											merged = A.Array.dedupe(merged).sort();
					...
					if (needsMetaSync.length) {
						...
						console.log((INDENT + needsMetaSync.join('\n' + INDENT)).warn);
					...
					if (needsModuleData.length) {
						...
						console.log((INDENT + needsModuleData.join('\n' + INDENT)).warn);
					...
					if (needsFileData.length) {
						...
						console.log((INDENT + needsFileData.join('\n' + INDENT)).warn);
					...
					if (VERBOSE && moduleInfo.missing.length) {
						...
						console.log((INDENT + moduleInfo.missing.join('\n' + INDENT)).warn);

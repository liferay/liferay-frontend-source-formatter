...
var re = {
	...
	html: {
		idFormat: {
			message: 'ID attribute values should be camelCased: {1}',
			regex: /\bid=(['"])(.*?)\1/i,
			test: function(item, regex, rule) {
				...
				item.replace(
					regex,
					function(str, quote, value) {
						if (re.test(value, re.REGEX_INVALID_ID_CHARS)) {
							invalid = true;
						}
					}
				);
